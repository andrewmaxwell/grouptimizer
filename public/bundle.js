...
/***/ function(module, exports, __webpack_require__) {
	...
	var initialTemperature = 5;
	var numIterations = 500 * 1000;
	...
	var rand = _require.rand;
	var standardDeviation = _require.standardDeviation;

	var StatGraph = __webpack_require__(4);

	var stats = new StatGraph(document.getElementById('statCanvas'));
	var annealingGraph = stats.addGraph({ color: 'red' });
	var temperatureGraph = stats.addGraph({ color: 'green', forceMin: 0 });

	var dataUrl = '//spreadsheets.google.com/feeds/list/1GFt1QV-LEui12pWztIMXwblcoLy5xdkTQBSQrlg50GY/od6/public/values?alt=json-in-script&callback=?';
	var getPeople = __webpack_require__(5);

	var solver = window.top.solver = new SimulatedAnnealingSolver({
		getCost: function getCost(grouping) {

			var genderRatios = []; // we want the genders to be balanced
			var contribAverages = []; // we want each group to have a similar average contrib
			var contribSDs = []; // we want to minimize standard deviation of of contrib standard deviation of each group
			...
			for (var i = 0; i < grouping.length; i++) {
				...
				group.stats = [genderRatios[i], contribAverages[i], contribSDs[i], weightSum];
			}

			var genderSD = standardDeviation(genderRatios);
			var avContribSD = standardDeviation(contribAverages);
			var contribSDSD = standardDeviation(contribSDs);
			var score = genderSD * 10 + avContribSD * 5 + contribSDSD - weightCost + 2;

			// grouping.stats = {genderSD, avContribSD, contribSDs, weightCost, score};

			return score;
		},
		generateNeighbor: function generateNeighbor(grouping) {
			// pick two random groups and swap a random person from each
			var newGrouping = grouping.slice(0);

			var groupIndex1 = rand(newGrouping.length);
			var groupIndex2 = void 0;
			do {
				groupIndex2 = rand(newGrouping.length);
			} while (groupIndex2 == groupIndex1);

			var group1 = newGrouping[groupIndex1] = newGrouping[groupIndex1].slice(0);
			var group2 = newGrouping[groupIndex2] = newGrouping[groupIndex2].slice(0);

			var personIndex1 = rand(group1.length);
			var personIndex2 = void 0;
			do {
	...
	var loop = function loop() {
		...
		for (var i = 0; !solver.isDone && i < iterationsPerFrame; i++) {
			solver.iterate();
		}

		if (solver.isDone) {
			$('#output').html(solver.bestState.map(function (g) {
				return g.sort(function (a, b) {
					return b.sponsor - a.sponsor || b.contrib - a.contrib;
				}).map(function (p) {
					return p.name;
				}).join(', ') + ' (' + g.stats.map(function (v) {
	...
	$('#go').on('click', function () {
		var numGroups = parseFloat($('#numGroups').val()) || 4;

		getPeople(dataUrl).then(function (people) {
...
/***/ function(module, exports) {
	...
	var SimulatedAnnealingSolver = function () {
		...
		_createClass(SimulatedAnnealingSolver, [{
			key: "init",
			value: function init(initialState, initialTemperature, maxIterations) {
...
/***/ function(module, exports) {
	...
	var variance = function variance(nums) {
	...
	module.exports = {
		...
		variance: variance,
		standardDeviation: function standardDeviation(nums) {
	...
	var StatGraph = function () {
		...
		_createClass(StatGraph, [{
			...
			value: function draw() {
				...
				this.graphs.filter(function (g) {
					return g.data.length;
				}).forEach(function (g, i) {
					T.fillStyle = T.strokeStyle = g.color;
					T.beginPath();
					for (var _i = 0; _i < g.data.length; _i++) {
						T.lineTo(_i / g.data.length * W, H - (g.data[_i] - g.min) / (g.max - g.min) * H);
					}
					// for (let x = 0; x <= W; x+=2){
...
/***/ function(module, exports, __webpack_require__) {
	...
	var dataFields = ['name', 'gender', 'absent', 'contrib', 'weights', 'sponsor'];

	module.exports = function (dataUrl) {
		return $.getJSON(dataUrl).then(function (data) {
			var people = data.feed.entry.map(function (row) {
				var person = {};

				dataFields.forEach(function (field) {
					var val = row['gsx$' + field].$t;
					var num = parseFloat(val);
					person[field] = isNaN(num) ? val : num;
				});

				person.weights = person.weights.split(/\s*,\s*/).reduce(function (weights, nameAndValue) {
					var _nameAndValue$split = nameAndValue.split(/\s*:\s*/);

					var _nameAndValue$split2 = _slicedToArray(_nameAndValue$split, 2);

					var person2Name = _nameAndValue$split2[0];
					var weight = _nameAndValue$split2[1];

					if (person2Name) {
						weights[person2Name] = parseFloat(weight);
					}
					return weights;
				}, {});
			...
			people.forEach(function (person1) {
				Object.keys(person1.weights).forEach(function (person2Name) {
					var person2 = people.find(function (p) {
						return p.name == person2Name;
					});
					if (person2) {
